...
% ignore = ["D:\Sensys Data\May132023\9AM",...
%  "/media/atsutse/Seagate Portable Drive/Sensys Data/May132023/9AM",...
%  "D:\Sensys Data\Jun022023\9PM",...
%  "/media/atsutse/Seagate Portable Drive/Sensys Data/Jun022023/9PM",...
%  ];

c = parcluster('local'); % build the 'local' cluster object
nw = c.NumWorkers;        % get the number of workers
par_check = gcp('nocreate');
if isempty(par_check)
    if(num_folder<nw)
        parpool(num_folder)
    else
        parpool(nw)
...
%                 cd(Fruit_folder);
                for q = 1:fruit_num
                      Fruit_filename = strcat(Fruit,num2str(q),'_','NT','_',day_num,'_',interval_suffix(j));
                      ...
                      if isfile(Fruit_filename_cvs)
                        ...
                        TP_Matrix(i,j,k,q) = sum(Fruit_data.*Fruit_data) - BG_power;
                      else
...
cd(home_dir)
if (BG_sub)
    save('Total_power_info.mat')
else
    save('Total_power_NBG_info.mat')
end
toc
%% Helper Function
